    ...
    implements Module, ModuleSpi, Activation
{
    ...
    private volatile EntityStore store;
    private volatile IdentityGenerator generator;
    private volatile Serialization serialization;
    private volatile MetricsProvider metrics;
    private volatile UnitOfWorkFactory uowf;
    ...
    public <T> ValueBuilder<T> newValueBuilderWithPrototype(T prototype)
    {
        ...
        if(valueType == null)
        {
            throw new IllegalArgumentException("Prototype '" + prototype + "' is not a valid composite. It has no types.");
        }
        ...
        throws NoSuchValueTypeException, ConstructionException
    {
        ...
        try
        {
            return serialization().deserialize(model.module(), Options.DEFAULT, model.valueType(), serializedState);
        }
