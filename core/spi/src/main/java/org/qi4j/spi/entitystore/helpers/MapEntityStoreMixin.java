 ...
 * <p>See {@link MemoryMapEntityStoreMixin} for reference.</p>
    ...
    implements EntityStore, EntityStoreSPI, StateStore, MapEntityStoreActivation
{
        ...
        throws EntityStoreException
    {
        try
        {
            ...
            ModuleDescriptor module = entityType.module();
            entityType.state().properties().forEach(
                persistentProperty ->
                {

                    Object value = state.properties().get(persistentProperty.qualifiedName());
                    JsonValue jsonValue = jsonSerialization.toJson(module, Options.ENTITY_STORAGE, value);
        ...
        throws EntityStoreException
    {
        try(JsonReader jsonReader = jsonFactories.readerFactory().createReader(entityState))
        {
            ...
            else
            {
                if(migration != null)
                {
                    state = migration.migrate(module, jsonStateBuilder.build(), application.version(), this);
                }
                else
                {
            ...
            entityDescriptor.state().properties().forEach(
                property ->
                {
                    try
                    {
                        ...
                        Object value = jsonSerialization.fromJson(module, Options.ENTITY_STORAGE, property.valueType(), jsonValue);
