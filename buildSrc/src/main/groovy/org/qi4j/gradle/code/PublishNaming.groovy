...
class PublishNaming
{
  static String publishedNameFor( String projectPath )
  {
    ...
    if( path[ 0 ] == 'libraries' )
    {
      return "org.qi4j.library.${ toName( path ) }"
    }
    else if( path[ 0 ].endsWith( 's' ) )
    {
      return "org.qi4j.${ path[ 0 ].substring( 0, path[ 0 ].length() - 1 ) }.${ toName( path ) }"
    }
    else if( path.size() > 1 )
    {
      return "org.qi4j.${ path[ 0 ] }.${ toName( path ) }"
    }
    return "org.qi4j.${ path[ 0 ] }"
  }
}
