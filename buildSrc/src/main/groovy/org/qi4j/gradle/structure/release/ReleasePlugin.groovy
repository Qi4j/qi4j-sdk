...
class ReleasePlugin implements Plugin<Project>
{
  ...
  static void checkAsfPreconditions( Project project )
  {
    ...
    {
      throw new InvalidUserDataException(
        'To perform ASF releases you need to clone the `qi4j-website` repository under ../qi4j-website' )
    }
    def qi4jDist = new File( project.rootProject.projectDir.parentFile, 'qi4j-dist' )
    if( !qi4jDist.exists() )
    {
      throw new InvalidUserDataException(
        'To perform releases you need to checkout the SVN dist directory under ../qi4j-dist' )
    }
  ...
  static Task applyPublishAsfMavenArtifacts( Project project )
  {
    ...
    def prepare = project.tasks.create( TaskNames.PREPARE_ASF_MAVEN, Zip ) { Zip task ->
      ...
      task.baseName = "qi4j-sdk-${ project.version }-maven-artifacts"
