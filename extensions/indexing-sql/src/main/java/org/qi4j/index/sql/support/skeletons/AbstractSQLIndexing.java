...
import org.qi4j.library.sql.generator.grammar.modification.DeleteStatement;
...
import org.qi4j.library.sql.generator.grammar.modification.UpdateStatement;
...
{
    private static final Logger LOGGER = LoggerFactory.getLogger( AbstractSQLIndexing.class );
...
//                    }
    ...
    private static void logStatement(String x, PreparedStatement st1)
    {
        try
        {
            Method m1 = st1.getClass().getDeclaredMethod("getDelegatePreparedStatement");
            m1.setAccessible(true);
            Object ps = m1.invoke(st1);
            Field f1 = ps.getClass().getSuperclass().getDeclaredField("statement");
            f1.setAccessible(true);
            Object st2 = f1.get(ps);
            Field f2 = st2.getClass().getSuperclass().getDeclaredField("batchParameters");
            f2.setAccessible(true);
            List r = (List) f2.get(st2);
            LOGGER.debug(x + st1 + "\n" + r.stream().map(Object::toString).collect(Collectors.joining("\n")));
        }
        catch(Exception e)
        {
            // ignore
        ...
        throws SQLException
    {
        ...
        StateHolder state = Qi4jAPI.FUNCTION_COMPOSITE_INSTANCE_OF.apply( (ValueComposite) property ).state();
