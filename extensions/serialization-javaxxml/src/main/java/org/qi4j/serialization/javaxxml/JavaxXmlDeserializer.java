    ...
    implements XmlDeserializer, Initializable
{
    ...
    public <T> T deserialize(ModuleDescriptor module, Options options, ValueType valueType, Reader state)
    {
        try
        {
            ...
            return fromXml(module, options, valueType, node);
        }
    ...
    @Override
    public <T> T fromXml(ModuleDescriptor module, Options options, ValueType valueType, Node state)
    {
        ...
        if(stateElement.isPresent())
        {
            ...
            return doDeserialize(module, options, valueType, stateNode.orElse(null));
        }
        return null;
    }

    @SuppressWarnings("unchecked")
    private <T> T doDeserialize(ModuleDescriptor module, Options options, ValueType valueType, Node xml)
    {
        if(xml == null)
        {
        ...
        if(converter != null)
        {
            return (T) converter.fromString(doDeserialize(module, options, ValueType.STRING, xml).toString());
        }
        JavaxXmlAdapter<?> adapter = adapters.adapterFor(valueType);
        if(adapter != null)
        {
            return (T) adapter.deserialize(module, options, xml, (element, type) -> doDeserialize(module, options, type, element));
        }
        ...
        {
            return (T) deserializeArray(module, options, (ArrayType) valueType, xml);
        }
        if(CollectionType.class.isAssignableFrom(valueTypeClass))
        {
            return (T) deserializeCollection(module, options,  (CollectionType) valueType, xml);
        }
        if(MapType.class.isAssignableFrom(valueTypeClass))
        {
            return (T) deserializeMap(module, options, (MapType) valueType, xml);
        }
        if(StatefulAssociationValueType.class.isAssignableFrom(valueTypeClass))
        {
            return (T) deserializeStatefulAssociationValue(module, options, (StatefulAssociationValueType<?>) valueType, xml);
        }
        return (T) doGuessDeserialize(module, options, valueType, xml);
    }
                                                       ...
                                                       StatefulAssociationValueType<?> valueType, Node xml)
    {
        ...
        ValueBuilder<?> builder = module.instance().newValueBuilderWithState(
            ...
            propertyFunction(valueType.module(), options, xml),
            associationFunction(valueType.module(), options, xml),
            manyAssociationFunction(valueType.module(), options, xml),
            namedAssociationFunction(valueType.module(), options, xml));
        return builder.newInstance();
    }

    private Function<PropertyDescriptor, Object> propertyFunction(ModuleDescriptor module, Options options, Node xml)
    {
        return property ->
        {
            ...
            if(element.isPresent())
            {
                ...
                if(converter != null)
                {
                    value = converter.fromString(doDeserialize(module, options, ValueType.STRING, valueNode));
                }
                else
                {
                    value = doDeserialize(module, options, property.valueType(), valueNode);
                }
    ...
    private Function<AssociationDescriptor, EntityReference> associationFunction(ModuleDescriptor module, Options options, Node xml)
    {
        return association ->
            (EntityReference) JavaxXml.firstChildElementNamed(xml, association.qualifiedName().name())
                .map(element -> doDeserialize(module, options,
    ...
    @SuppressWarnings("unchecked")
    private Function<AssociationDescriptor, Stream<EntityReference>> manyAssociationFunction(ModuleDescriptor module, Options options, Node xml)
    {
        return association ->
            JavaxXml.firstChildElementNamed(xml, association.qualifiedName().name())
                .map(element -> (List) doDeserialize(module, options,
    ...
    private Function<AssociationDescriptor, Stream<Map.Entry<String, EntityReference>>> namedAssociationFunction(
        ModuleDescriptor module, Options options, Node xml)
    {
        return association ->
            (Stream) JavaxXml.firstChildElementNamed(xml, association.qualifiedName().name())
                .map(element -> (Map) doDeserialize(module, options,
    ...
    private Object deserializeArray(ModuleDescriptor module,Options options,  ArrayType arrayType, Node xml)
    {
        ...
        List collection = (List) deserializeCollection(module, options, collectionType, xml);
    ...
    @SuppressWarnings("unchecked")
    private Collection deserializeCollection(ModuleDescriptor module, Options options, CollectionType collectionType, Node xml)
    {
        ...
        return JavaxXml
            ...
            .map(element ->
            {
                ...
                {
                    return doDeserialize(module, options, collectionType.collectedType(),
                        JavaxXml.firstStateChildNode(element).get());
                }
                return doDeserialize(module, options, collectionType.collectedType(), element);
            })
            .collect(collection);
    }

    @SuppressWarnings("unchecked")
    private Map deserializeMap(ModuleDescriptor module, Options options, MapType mapType, Node xml)
    {
        if(!xml.hasChildNodes())
        {
            ...
            element ->
            {
                if(complexMapping.test(element))
                {
                    ...
                    Optional<Node> valueNode = JavaxXml.firstChildElementNamed(element, "value")
                        .flatMap(JavaxXml::firstStateChildNode);
                    Object key = doDeserialize(module, options, mapType.keyType(), keyNode);
                    Object value = valueNode.map(node -> doDeserialize(module, options, mapType.valueType(), node))
                        .orElse(null);
                    return new HashMap.SimpleImmutableEntry<>(key, value);
                }
                String key = element.getTagName();
                Object value = JavaxXml.firstStateChildNode(element)
                    .map(node -> doDeserialize(module, options, mapType.valueType(), node))
                    .orElse(null);
    ...
    private Object doGuessDeserialize(ModuleDescriptor module, Options options, ValueType valueType, Node xml)
    {
        // TODO Could do better by detecting <collection/>, <map/> and <value/>
        Optional<String> typeInfo = getTypeInfo(xml);
        if(typeInfo.isPresent())
        {
            StatefulAssociationCompositeDescriptor descriptor = statefulCompositeDescriptorFor(module,typeInfo.get());
            if(descriptor != null)
            {
                return deserializeStatefulAssociationValue(((CompositeDescriptor) descriptor).module(), options,
