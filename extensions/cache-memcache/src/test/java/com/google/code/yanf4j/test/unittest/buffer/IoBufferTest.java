 ...
 * @version $Rev: 754874 $, $Date: 2009-03-16 12:04:01 +0100 (Mon, 16 Mar 2009) $
 */
public class IoBufferTest {

  /*
   * public static void main(String[] args) { junit.textui.TestRunner.run(IoBufferTest.class); }
   */
  ...
  public void testGetStringWithFailure() throws Exception {
    ...
    try {
      buffer.getString(3, StandardCharsets.US_ASCII.newDecoder());
      fail();
    } catch (Exception e) {
    ...
    try {
      buffer.getString(StandardCharsets.US_ASCII.newDecoder());
      fail();
    } catch (Exception e) {
      assertEquals(oldLimit, buffer.limit());
      assertEquals(oldPos, buffer.position());
